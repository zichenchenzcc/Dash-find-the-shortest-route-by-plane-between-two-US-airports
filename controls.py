...
import numpy as np
import math
os.chdir("C:\\Users\\czc\\Desktop\\Kartemap-Zichen Chen, Changtao Shuai")
...
def path_to_list(path_airport,path_airport1,path_airport2):
    path_list = [path_airport,path_airport1,path_airport2]
    return path_list

def to_total_path(path_list):
    total_path = path_list[0]
    if len(path_list)>1:
        for i in range(1,len(path_list)):
            copy = path_list[i].copy()
            del copy[0]
            total_path = total_path + copy
    return total_path